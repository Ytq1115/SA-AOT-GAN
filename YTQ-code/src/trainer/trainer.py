...
if not hasattr(pil_image, "ANTIALIAS"):
    pil_image.ANTIALIAS = getattr(pil_image, "Resampling", pil_image).LANCZOS
...
from .common import timer, sobel_edge
...
class Trainer:
    def __init__(self, args):
        ...
        # # ---- A-1 soft structure constraint (optional) ----
        # self.use_edge = bool(getattr(args, "use_edge", False))
        # self.edge_weight = float(getattr(args, "edge_weight", 1.0))
        # self.edge_loss_fn = nn.BCEWithLogitsLoss(reduction="none")
        ...
        # ✅ 方案A：soft target + L1
        # 这里不再用 BCEWithLogitsLoss；我们将对 sigmoid(edge_logits) 做 L1 回归
        self.edge_loss_fn = None  # placeholder, keep structure minimal
    ...
    def train(self):
        ...
        for idx in pbar:
            ...
            # in: [rgb(3) + edge(1)]
            if self.use_edge:
                pred_img, edge_logits = self.netG(images_masked, masks, return_edge=True)
            else:
            ...
            # # ---- A-1 edge supervision (optional) ----
            # if self.use_edge:
            #     # edge_gt from GT image (no grad)
            #     with torch.no_grad():
            #         edge_gt = sobel_edge(images, thr=0.1)  # [B,1,H,W] in {0,1}
            #
            #     # BCEWithLogits per-pixel
            #     edge_loss_map = self.edge_loss_fn(edge_logits, edge_gt)
            #
            #     # only supervise inside holes (mask=1 is hole in your pipeline)
            #     edge_loss_map = edge_loss_map * masks
            #
            #     # normalize by number of supervised pixels (avoid scale drift)
            #     denom = masks.sum().clamp_min(1.0)
            #     l_edge = edge_loss_map.sum() / denom
            #
            #     progress = float(self.iteration) / float(self.args.iterations)
            #     edge_w = self.edge_weight * max(0.2, 1.0 - progress)  # 0.2 可改 0.1~0.3
            #     losses["edge"] = l_edge * edge_w
            #
            # # adversarial loss
            ...
            #
            #     if self.args.tensorboard and self.use_edge:
            #         # sigmoid -> probability map for visualization
            #         edge_prob = torch.sigmoid(edge_logits)
            #         self.writer.add_image("edge_gt", make_grid(edge_gt), self.iteration)
            #         self.writer.add_image("edge_pred", make_grid(edge_prob), self.iteration)
            #
            #
            ...
            if self.use_edge:
                # ✅ 方案A：edge_gt 是 soft target（0~1）
                with torch.no_grad():
                    edge_gt = sobel_edge(images, thr=0.1)  # [B,1,H,W] in [0,1] (thr 参数保留但不影响soft)

                # ✅ sigmoid logits -> prob map in [0,1]
                edge_pred = torch.sigmoid(edge_logits)

                # ✅ L1 per-pixel: |pred - target|
                edge_loss_map = torch.abs(edge_pred - edge_gt)
            ...
            if self.args.global_rank == 0 and (self.iteration % self.args.print_every == 0):
                ...
                if self.args.tensorboard and self.use_edge:
                    # edge_pred 已经是 sigmoid 后的概率图
                    self.writer.add_image("edge_gt", make_grid(edge_gt), self.iteration)
                    self.writer.add_image("edge_pred", make_grid(edge_pred), self.iteration)
